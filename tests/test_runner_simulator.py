...
from masterblaster_control.p0_policy import REASON_ALLOW, REASON_TARGET_OUT_OF_SCOPE, canonical_json
...
def test_runner_emits_deterministic_hashed_fixture_evidence():
    ...
    first = runner.run("a0.fixture.inventory", "example.com", now=now)
    second = runner.run("a0.fixture.inventory", "example.com", now=now)
...
def test_tls_fake_transport_completes_for_domain_target():
    ...
    result = runner.run("a1.tls.assessment", "example.com", now=now)
